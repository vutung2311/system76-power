...
const UDEV_PM_RULES_PATH: &str = "/usr/lib/udev/rules.d/80-nvidia-pm.rules";

static UDEV_PM_RULES: &[u8] = br#"# Automatically generated by system76-power
...
pub enum GraphicsDeviceError {
    ...
    Rescan(io::Error),
    #[error(display = "filed to modify to NVIDIA udev rules: {}", _0)]
    UdevRules(io::Error),
...
impl Graphics {
    ...
    pub fn set_vendor(&self, vendor: &str) -> Result<(), GraphicsDeviceError> {
        ...
        if vendor == "hybrid" {
            info!("Creating udev rules for NVIDIA power management");
            fs::write(UDEV_PM_RULES_PATH, UDEV_PM_RULES)
                .map_err(GraphicsDeviceError::UdevRules)?;
        } else {
            if Path::new(UDEV_PM_RULES_PATH).exists() {
                info!("Removing udev rules for NVIDIA power management");
                fs::remove_file(UDEV_PM_RULES_PATH)
                    .map_err(GraphicsDeviceError::UdevRules)?;
            }
        }

        info!("Reloading udev rules");
        let status = process::Command::new("udevadm")
            .arg("control")
            .arg("--reload-rules")
            .status()
            .map_err(|why| GraphicsDeviceError::Command {cmd: "udevadm", why})?;

        if !status.success() {
            warn!("udevadm: failed to reload rules");
        }
